...
function CancelAskFriend(props: any) {
    const authCtx = React.useContext(AuthChecker);
    const friendsCtx = React.useContext(FriendsContext);

    async function handleCancelAskFriend(){
        friendsCtx.cancelRequest(props.loadedUser!.id)
        ...
        if (params !== null) {
            fetch(
                ...
                .then(_response => { props.onCancelAskFriend(props.loadedUser!.id) })
                .catch(_error => console.error("error in deny friend request"));
        }
    }
    return (
        <div>
            <button onClick={handleCancelAskFriend} title={'Cancel friend request'}> <DoDisturbIcon/></button>
        </div>
    );
}

export default CancelAskFriend;